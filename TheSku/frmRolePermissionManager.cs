...
namespace TheSku
{
    public partial class frmRolePermissionManager : Form
    {
        ...
        public frmRolePermissionManager(AppDbContext dbContext)
        {
            ...
            this.cmbRole.DataSource = dbContext.Roles.Where(r => !r.Disabled).ToList();
            this.cmbDocument.DataSource = dbContext.Documents.ToList();
        }
        ...
        private void gvList_CellValueChanged(object sender, GridViewCellEventArgs e)
        {
            if (e.ActiveEditor is RadCheckBoxEditor)
            {
                ...
                if (permission != null)
                {
                    permission.Modified = Utility.Now;
                    permission.ModifiedBy = Global.User.Name;
        ...
        private void cmbDocument_SelectedIndexChanged(object sender, Telerik.WinControls.UI.Data.PositionChangedEventArgs e)
        {
            if (this.cmbRole.SelectedIndex >= 0 && this.cmbDocument.SelectedIndex >= 0)
            {
                this.gvList.DataSource = dbContext.UserPermissions.Where(p => p.Role.Equals(dbContext.Roles.Where(r => r.Name.Equals(this.cmbRole.SelectedValue)).FirstOrDefault()) && p.DocumentType.Equals(this.cmbDocument.SelectedValue)).OrderBy(p => p.DocumentType).ToList();
            }
            else if (this.cmbRole.SelectedIndex >= 0)
            {
                this.gvList.DataSource = dbContext.UserPermissions.Where(p => p.Role.Equals(dbContext.Roles.Where(r => r.Name.Equals(this.cmbRole.SelectedValue)).FirstOrDefault())).OrderBy(p => p.DocumentType).ToList();
            }
